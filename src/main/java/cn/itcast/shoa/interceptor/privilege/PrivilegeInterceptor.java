...
public class PrivilegeInterceptor implements Interceptor {
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		/**
		 * 1、获取session中的用户能够访问的权限 2、获取当前访问action中的方法的权限
		 * 3、检查该用户是否具有action中的方法的访问权限 前提条件：如果用户名称为"admin",则放行 如果有 则访问action 如果没有
		 * 则跳转到错误处理页面
		 */
		...
		if ("admin".equals(user.getUsername())) {
			...
			String privilegeName = PrivilegeInfoParse.parse(classAction,
					methodName);
